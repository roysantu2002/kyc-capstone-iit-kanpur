...
export const Owner = () => {
    ...
    const [owner, setOwner] = useState("Contract Owner Details");
      const {
    handleSubmit,
  } = useForm();
  ...
  const onSubmit = async () => {
    ...
    try {
      ...
      const _owner = web3.utils.keccak256(result)
      setOwner(result)
      console.log(_owner)
  ...
  <Section yPadding="pt-16 pb-16">
          ...
          <div className="flex flex-col max-w-4xl mx-auto overflow-hidden bg-white rounded-lg shadow-lg dark:bg-gray-800 md:flex-row md:h-48">
            ...
            <div className="px-6 py-6 md:px-8 md:py-0">
                ...
                <p className="mt-2 text-sm text-gray-600 dark:text-gray-400 md:text-gray-400">get the contract owner</p>
            </div>
        </div>
       

        <div className="flex items-center justify-center pb-6 md:py-0 md:w-2/3">
            ...
            <form className='' onSubmit={handleSubmit(onSubmit)}>
              ...
              placeholder={owner}/> */}
                    ...
                    <button className="px-4 py-3 text-sm font-medium tracking-wider text-gray-100 uppercase transition-colors duration-200 transform bg-primary-500 rounded-lg hover:bg-gray-600 focus:bg-gray-600 focus:outline-none"> {loading ? <Loader /> : "Get Owner"}</button>
                </div>
